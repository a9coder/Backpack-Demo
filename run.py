...
def parse_arguments():
    ...
    parser.add_argument('--next-round-delay', type=float, default=3.0, help='純Maker策略一輪結束後等待秒數 (默認: 3.0)')
...
def main():
    ...
    if args.web:
        ...
        try:
            ...
            if strategy_name == 'pure_maker' and market_type != 'perp':
                logger.error("純 Maker-Maker 策略目前僅支持永續合約，請使用 --market-type perp")
                sys.exit(1)
            ...
            if strategy_name == 'grid':
                ...
                if strategy_name == 'maker_hedge':
                    ...
                    logger.info("啟動純 Maker-Maker 刷量策略 (永續合約)")
                    logger.info(f"  單筆數量: {args.quantity}")
                    logger.info(f"  最大倉位: {args.max_position}")
                    logger.info(f"  加倉價格步長: {args.scale_in_price_step_pct}%")
                    logger.info(f"  加倉倉位比例: {args.scale_in_size_pct}%")
                    logger.info(f"  輪次間隔: {args.next_round_delay} 秒")
                    market_maker = PureMakerStrategy(
                        ...
                        next_round_delay_seconds=args.next_round_delay,
                        exchange=exchange,
                        exchange_config=exchange_config,
                        enable_database=args.enable_db,
                    )
